...
test('multiple-cut audition preserves the chosen middle take and skips both alternatives', () => {
  const fake = useFake()
  player.auditionCuts([{ start: 2, end: 4 }, { start: 7, end: 9 }], 0.5)
  assert.equal(fake.currentTime, 1.5)
  fake.advance(2)
  assert.equal(fake.currentTime, 4)
  fake.advance(7)
  assert.equal(fake.currentTime, 9)
  fake.advance(9.5)
  assert.equal(fake.playCalls.length, 3)
  player.set(null)
})
