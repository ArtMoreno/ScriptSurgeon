...
/** Preserve delivered bytes even when capture ended with an error. */
export function capturedAudio(chunks: Blob[], type: string): Blob | null {
  const blob = new Blob(chunks.filter(chunk => chunk.size > 0), { type })
