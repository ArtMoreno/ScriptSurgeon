...
function isRapidRestart(before: Word[], after: Word[], pause: number): boolean {
  ...
  return alignment.shared >= 2
    ...
    && (alignment.prefix >= 3 || alignment.suffix >= 2)
}
 ...
 * quotes, speaker changes and explicit keep decisions constrain detection.
 */
export function detectRetakes(
  ...
  // Resolve the innermost restart first. A later review can then compare the
  // cleaned take with an earlier attempt without overlapping cut choices.
  const clauseProposals: CleanupProposal[] = []
  const clauseCandidates: CleanupProposal[] = []
  const clauseWords = new Set<string>()
  const openings = new Map<string, number[]>()
  for (let i = 0; i + 6 <= kept.length; i += 1) {
    const key = normalized.slice(i, i + 6).join(' ')
    const positions = openings.get(key) ?? []
    positions.push(i)
    openings.set(key, positions)
  }
  // ponytail: local, bounded lexical evidence, not a semantic intent model.
  for (let later = kept.length - 6; later >= 0; later -= 1) {
    let end = later
    while (end < kept.length - 1 && end - later < 47 && !hasSentenceBoundary([kept[end]])) end += 1
    const after = kept.slice(later, end + 1)
    if (after.length < 9 || !hasSentenceBoundary([after[after.length - 1]])) continue
    const laterOpenings = after.slice(0, -5).map((_, i) => normalized.slice(later + i, later + i + 6).join(' '))
    if (new Set(laterOpenings).size !== laterOpenings.length) continue
    const starts = openings.get(normalized.slice(later, later + 6).join(' ')) ?? []
    for (const earlier of [...starts].reverse()) {
      if (earlier >= later || later - earlier < 6 || later - earlier > 96) continue
      if (kept[later].startTime - kept[earlier].startTime > 60) continue
      const before = kept.slice(earlier, later)
      const attempts = [...before, ...after]
      if (hasSentenceBoundary(before) || hasQuoteBoundary(attempts)
        || crossesSpeakerBoundary(attempts, speakers)
        || attempts.some(w => clauseWords.has(w.id))) continue
      candidateWindows += 1
      const alignment = retakeAlignment(before, after)
      if (alignment.shared < 8 || alignment.shared / after.length < 0.65) continue
      const proposal = proposalForWords('retakes', words, before.map(w => w.id), 'medium',
        'Repeated incomplete passage followed by a complete ending; review this inner restart first', after.map(w => w.id), options)
      if (proposal) {
        clauseCandidates.push(proposal)
      }
      break
    }
  }
  // Prefer the longest complete later passage, not a shared phrase buried
  // near its ending. Otherwise a valid suffix can leave a broken lead-in.
  clauseCandidates.sort((a, b) => b.retakeGroup!.candidates[1].wordIds.length - a.retakeGroup!.candidates[1].wordIds.length
    || a.wordIds.length - b.wordIds.length)
  for (const proposal of clauseCandidates) {
    const ids = proposal.retakeGroup!.candidates.flatMap(c => c.wordIds)
    if (ids.some(id => clauseWords.has(id))) continue
    ids.forEach(id => clauseWords.add(id))
    // Reserve the full matched passage even when protected, so a shorter
    // suffix cannot evade a keep decision on the beginning of this take.
  ...
  for (let boundary = 2; boundary < kept.length - 3; boundary += 1) {
    const pause = kept[boundary + 1].startTime - kept[boundary].endTime
    if (pause < RETAKE_PAUSE || pause > 5) continue
    let first = boundary
    while (first > 0 && boundary - first < 31
      && !hasSentenceBoundary([kept[first - 1]])
      && kept[first].startTime - kept[first - 1].endTime < RETAKE_PAUSE) first -= 1
    // Never turn a bounded suffix into a supposedly complete attempt.
    if (first > 0 && !hasSentenceBoundary([kept[first - 1]])
      && kept[first].startTime - kept[first - 1].endTime < RETAKE_PAUSE) continue
    let last = boundary + 1
    while (last < kept.length - 1 && last - boundary < 32
      && !hasSentenceBoundary([kept[last]])
      && kept[last + 1].startTime - kept[last].endTime < RETAKE_PAUSE) last += 1
    if (last < kept.length - 1 && !hasSentenceBoundary([kept[last]])
      && kept[last + 1].startTime - kept[last].endTime < RETAKE_PAUSE) continue
    const before = kept.slice(first, boundary + 1)
    ...
    candidateWindows += 1
    const alignment = retakeAlignment(before, after)
    if (alignment.prefix < 3 || alignment.shared < 3
      || alignment.shared / before.length < 0.8
      || (hasSentenceBoundary(before) && alignment.shared / after.length < 0.8)) continue
    const proposal = proposalForWords('retakes', words, before.map(w => w.id), 'medium',
      'Nearby sentence restart; compare both complete attempts before choosing', after.map(w => w.id), options)
    if (proposal) {
      proposals.push(proposal)
      before.forEach(w => covered.add(w.id))
    }
  }
  ...
  for (let boundary = 2; boundary < kept.length - 3; boundary += 1) {
    ...
    if (pause < 0 || pause > 0.65) { reject('outside-immediate-window'); continue }
    ...
    for (let length = 5; length >= 3; length -= 1) {
      ...
      // Equal-length windows must not borrow words from a neighboring
      // sentence or bridge another pause just to increase their match score.
      if ([before, after].some(attempt => hasSentenceBoundary(attempt.slice(0, -1))
        || attempt.slice(1).some((word, i) => word.startTime - attempt[i].endTime > 0.65))) continue
      if (
  ...
  for (let index = 0; index < kept.length; index += 1) {
    ...
    const ids = candidate.map((word) => word.id)
    if (ids.some(id => keepIds.has(id) || covered.has(id))) { reject('kept-or-overlapping'); continue }
    ...
    if (hasQuoteBoundary([...candidate, ...following])) { reject('quoted-content-or-speaker-boundary'); continue }
  ...
  const mergedProposals = [...clauseProposals, ...mergeRetakeChains(words, proposals.filter(p =>
    !p.retakeGroup?.candidates.some(c => c.wordIds.some(id => clauseWords.has(id)))), options)]
