...
import { runCleanup, selectRetakeCandidate } from './lib/cleanup.ts'
...
test('cut preview follows the chosen take and stays silent until a choice exists', () => {
  const words = 'We need help. We need help.'.split(' ').map((text, i) => ({ ...word, id: `preview${i}`, text, startTime: i * 0.4, endTime: i * 0.4 + 0.2 }))
  const preview = runCleanup('retakes', words, [], [], [], [])
  const proposal = preview.proposals[0]
  assert.ok(proposal?.retakeGroup)
  const old = useStore.getState()
  const audition = player.auditionCuts
  const calls: Array<Array<{ start: number; end: number }>> = []
  player.auditionCuts = cuts => { calls.push(cuts) }
  try {
    useStore.setState({ words, cleanupPreview: preview, waveformReady: true, rendering: false, audioPreviewMode: 'original' })
    useStore.getState().auditionCleanupProposal(proposal.id)
    assert.equal(calls.length, 0)
    const [earlier, later] = proposal.retakeGroup.candidates
    useStore.setState({ cleanupPreview: selectRetakeCandidate(preview, proposal.id, earlier.id) })
    useStore.getState().auditionCleanupProposal(proposal.id)
    assert.deepEqual(calls, [[{ start: later.startTime, end: later.endTime }]])
  } finally {
    player.auditionCuts = audition
    useStore.setState(old, true)
  }
})
