...
export default function TranscriptEditor({
  ...
  // A transcript that stops being ready cannot keep a context menu open.
  ...
  if (contextMenu?.target.kind === 'word') {
    ...
    if (clip) {
      // As above: the ref write happens in the onSelect handler, not in render.
      // eslint-disable-next-line react-hooks/refs
      menuActions.push({
        id: 'edit-insert-text',
        label: 'Edit text',
  ...
  return (
    <section className="flex-1 min-h-0 flex flex-col bg-canvas" aria-label="Transcript editor">
      <div className="transcript-tools min-h-10 shrink-0 px-5 py-1.5 flex flex-wrap items-center gap-3 text-[11px]">
