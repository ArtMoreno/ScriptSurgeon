...
import { AudioIcon, BackIcon, ForwardIcon, PauseIcon, PlayIcon } from './Icons'
...
export default function TransportBar({
  ...
  return (
    ...
    >
      ...
      <span className="transport-time" aria-label="Playback position">{fmtTime(playTime)} / {fmtTime(duration)}</span>
      <div
        data-transport-centerline
        className={`min-w-0 justify-self-center flex items-center ${thin ? 'gap-1.5' : 'gap-2.5'}`}
      >
        <button className="transport-record" onClick={onRecordInsert} disabled={status !== 'ready'}><AudioIcon /><span>Record</span></button>
