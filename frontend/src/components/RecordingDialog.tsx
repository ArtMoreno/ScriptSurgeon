...
export default function RecordingDialog(props: RecordingDialogProps) {
  ...
  const startMonitor = async () => {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: audioConstraints(deviceId, voiceProcessing),
      })
      ...
      if (mountedRef.current) setError(microphoneError(caught))
    }
  }
  ...
  useEffect(() => {
    ...
    // deviceId constraint throw, so fall back to the system default.
    if (!deviceId || !devices.length) return
    if (!devices.some((device) => device.deviceId === deviceId)) setDeviceId('')
  }, [devices, deviceId])
  ...
  const beginRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        const file = new File([blob], `${PRODUCT_FILE_STEM}-recording-${Date.now()}.${recordingExtension(type)}`, { type })
  ...
  const handleDialogKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    const focusable = Array.from(event.currentTarget.querySelectorAll<HTMLElement>(
      'button:not(:disabled), textarea:not(:disabled), input:not(:disabled), audio[controls]',
  ...
  return (
    ...
    >
      <div
        ...
        className="flex max-h-[calc(100vh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-line-strong bg-canvas-raised text-ink shadow-2xl shadow-ink/25 sm:max-h-[calc(100vh-3rem)]"
      >
                ...
                {!recording && (
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    >
                      ...
                      {monitoring ? 'Stop level check' : 'Check level'}
                    </button>
