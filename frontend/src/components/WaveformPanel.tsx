...
interface Menu {
  ...
  y: number
  /**
   * The container width at the moment the menu opened. Captured here so the
   * render pass never has to read layout off a ref, which can be stale.
   */
  hostWidth: number
  region: Region
}
...
export default function WaveformPanel({
  ...
  // Read by the WaveSurfer effects below, never during render.
  ...
  useEffect(() => {
    ...
    if (!audioUrl) {
      // Loading audio into WaveSurfer is external-system work, and this marks the
      // panel not-ready for the duration of it.
    ...
    >
      ...
      {!minimized && menu && (
        <div
          ...
          style={{ left: Math.max(8, Math.min(menu.x - 120, menu.hostWidth - 252)), top: menu.y + 34 }}
          role="toolbar"
