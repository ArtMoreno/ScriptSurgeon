...
param(
    ...
    # Matches the installer's per-user, no-elevation posture. The previous
    # default was one machine's own drive layout, which exists nowhere else.
    # Pass -InstallDirectory to choose somewhere else.
    [string]$InstallDirectory = (Join-Path $env:LOCALAPPDATA 'Programs\ScriptSurgeon'),
