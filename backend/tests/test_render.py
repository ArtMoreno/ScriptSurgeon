...
class RenderTimelineTests(unittest.TestCase):
    def test_wav_range_preserves_sample_rate_and_exact_pcm_slice(self):
        with tempfile.TemporaryDirectory() as temp:
            source = Path(temp) / "source.wav"
            output = Path(temp) / "clip.wav"
            write_tone(source, duration=2, rate=48_000)
            export_segment(str(source), str(output), "wav", start=0.5, end=1.5)
            with wave.open(str(source), "rb") as original, wave.open(str(output), "rb") as clip:
                self.assertEqual(clip.getframerate(), original.getframerate())
                self.assertEqual(clip.getnframes(), 48_000)
                original.setpos(24_000)
                self.assertEqual(clip.readframes(48_000), original.readframes(48_000))
